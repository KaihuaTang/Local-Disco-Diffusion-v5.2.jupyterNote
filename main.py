...
def do_run(args, model_path, gpu_device):
  seed = args.seed
          ...
          if turbo_mode:
            if frame_num == turbo_preroll: #start tracking oldframe
              ...
              if frame_num % int(turbo_steps) != 0: 
                ...
                if vr_mode:
                  generate_eye_views(args, TRANSLATION_SCALE,batchFolder,filename,frame_num,midas_model, midas_transform, gpu_device)
                continue
              else:
            ...
            with image_display:
              if j % args.display_rate == 0 or cur_t == -1 or intermediateStep == True:
                  for k, image in enumerate(sample['pred_xstart']):
                      ...
                      if cur_t == -1:
                        if frame_num == 0:
                          save_settings(args, batchFolder, batch_name, batchNum)
                        if args.animation_mode != "None":
